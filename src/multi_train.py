...
logging.basicConfig(
    filename="multitrain.log", level=logging.INFO, format="%(asctime)s %(message)s"
)
...
def run_retraining(
    ...
    try:
        ...
        if (
            abs(e.returncode) == signal.Signals.SIGSEGV
            or abs(e.returncode) == signal.Signals.SIGABRT
        ):
    ...
    try:
        ...
        if (
            ...
            if model_path == "":
                ...
                run_retraining(
                    model_path, server_name, config_file, log_file, timesteps
                )
        logging.info(
            f"Failed to resume training for {model} in RUN_TRAINING with error: {e}"
        )
...
def main():
    ...
    for i in range(total_runs):
        ...
        process = Process(
            target=run_training,
            args=(model, f"{args.base_carla_host}-{i+1}", args.timesteps, config_file),
        )
